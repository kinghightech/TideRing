...
///
/// One instance is created per connection by `RingManager` and shared with the driver's
/// `RingDecoder` and command flow, so the offset used to decode a reply is always the one that
/// connection actually sent. It is a reference type so both hold the same latched value.
