...
//  ringmvp
//
//  The JRing (SMART_RING, service 000056ff…) byte-level protocol.
//  Ported verbatim from PulseLoop by Saksham Bhutani
//  (https://github.com/saksham2001/PulseLoopiOS), licensed under
//  Creative Commons Attribution 4.0 International (CC BY 4.0).
//
//  This is the proven, on-hardware-tested encoder/decoder. Do not "improve" the byte
//  layouts — they are matched against a real ring and the vendor app's own behaviour.
//
...
enum RingCommandID: UInt8 {
    ...
    /// Ring-to-phone action packet. Byte [1] == 2 is the camera shutter gesture.
    case deviceAction = 0x06
    /// Enable/disable the ring's gesture-controlled photo mode.
    case photoMode = 0x07
...
enum RingDecodedEvent: Sendable {
    ...
    /// The ring detected its configured camera gesture while photo mode was active.
    case cameraShutter(timestamp: Date)
    /// Ring bind/unbind handshake frame (0x4B): `action`/`state` per the protocol state machine.
    case bind(action: UInt8, state: UInt8)
    /// Capability bitmask reply (0x20).
    ...
    var kind: String {
        ...
        case .cameraShutter: return "camera_shutter"
...
struct RingDecoder {
    ...
    /// epochs and must have it subtracted off; `nil` subtracts 0.
    ...
    func decodeAll(_ data: Data) -> [RingDecodedEvent] {
        ...
        // One 0x10 activity-history frame contains fifteen consecutive one-minute step buckets.
        // Collapse the frame into one timestamped 15-minute bucket; that preserves the exact daily
        // total while avoiding fifteen persistence writes for every BLE notification.
        if packet.commandId == 0x10, bytes.count >= 20 {
            return decodeActivityHistory(bytes)
        }
    ...
    /// Decode a 0x10 activity-history frame. Bytes [1...4] are the base timestamp and [5...19]
    /// are fifteen one-minute step buckets. The format carries no verified distance/calorie fields.
    private func decodeActivityHistory(_ bytes: [UInt8]) -> [RingDecodedEvent] {
        let base = ringDate(u32le(bytes, 1))
        let steps = bytes[5..<20].reduce(0) { $0 + Int($1) }
        return [.activityBucket(timestamp: base, steps: steps, distanceMeters: 0)]
    }
    ...
    func decode(_ data: Data) -> RingDecodedEvent {
        ...
        case 0x06 where bytes.count >= 2 && bytes[1] == 0x02:
            return .cameraShutter(timestamp: now)
        case 0x07:
            return .commandAck(commandId: packet.commandId)
        ...
        case 0x16 where bytes.count >= 2 && bytes[1] == 0xff:
            // End-of-stream marker for the measurement-history sync.
            return .historySyncFinished
        case 0x16 where bytes.count >= 2 && bytes[1] == 0xf0:
            // Header marker. Surfacing progress lets the sync watchdog distinguish an active stream
            // from a stalled one even before the first measurement block arrives.
            return .historySyncProgress(stage: "Syncing heart rate…")
        case 0x16 where bytes.count >= 2 && bytes[1] == 0xaa:
            return .historySyncProgress(stage: "Receiving heart-rate history…")
        case 0x16:
            // Data blocks (0xA0) are handled in `decodeAll`.
            return .commandAck(commandId: packet.commandId)
        ...
        case 0x28:
            // Blood-data notify: a measurement-complete marker.
...
struct RingEncoder {
    ...
    /// Request the ring's stored activity and sleep stream. The verified JRing request is exactly
    /// `0x10` followed by nineteen zero bytes; the ring decides which retained records to return.
    func makeHistoryQueryCommand() -> Data {
        data(hex: "1000000000000000000000000000000000000000")
    }
    ...
    /// Official JRing SDK `setPhontMode`: 0x07 + enabled byte, padded to the 20-byte 56ff frame.
    func makePhotoModeCommand(enabled: Bool) -> Data {
    ...
    /// App identity (0x48): claims the ring with a persistent app ID so it streams data to the app
